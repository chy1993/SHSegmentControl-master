...
public class SegmentControl extends View {
    ...
    public SegmentControl(Context context, AttributeSet attrs, int defStyle) {
        ...
        mColor = ta.getColor(R.styleable.SegmentControl_colors, Color.WHITE);
        mSelectedTextColor = ta.getColor(R.styleable.SegmentControl_textSelectedColors, Color.WHITE);
        mUnselectedTextColor = ta.getColor(R.styleable.SegmentControl_textUnselectedColors, Color.GRAY);
        mStrokeColor = ta.getColor(R.styleable.SegmentControl_stroke_color, Color.GRAY);

        mFillInset = ta.getDimensionPixelSize(R.styleable.SegmentControl_fill_inset, mFillInset);
        ...
        mSelectedDrawable = new RadiusDrawable(false);
               mSelectedDrawable.setStrokeColor(mStrokeColor);

        mSelectedDrawable.setFillColor(mColor);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mTexts != null && mTexts.length > 0) {
            ...
            if (mCacheBounds == null || mCacheBounds.length != mTexts.length) {
                ...
                mFillBounds = new Rect[mTexts.length];
            }
            ...
            for (int i = 0; i < mCacheBounds.length; i++) {
                Rect cacheBound = mCacheBounds[i];
                    mFillBounds[i] = new Rect(cacheBound);
                    mFillBounds[i].inset(mFillInset, mFillInset);
            }
        } else {
...
//                    mSelectedDrawable.setBounds(mCacheBounds[i]);
                    mSelectedDrawable.setBounds(mFillBounds[i]);
